...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_orders(self, page=1, date_begin=None, date_end=None, sortby=None, name_id=None, **kw):
        values = self._prepare_portal_layout_values()
        ...
        # 更改订单时间为订单创建时间排序
        searchbar_sortings = {
            'date': {'label': _('Order Create Date'), 'order': 'create_date desc'},
            # 'date': {'label': _('Order Create Date'), 'order': 'date_order desc'},
            'name': {'label': _('Reference'), 'order': 'name'},
            'stage': {'label': _('Stage'), 'order': 'state'},
        }
        # default sortby order
        ...
        # 添加订单号搜索功能
        if name_id:
            domain.append(("name", "ilike", name_id))
